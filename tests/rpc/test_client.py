...
@pytest.fixture
def rpc_client(client_settings):
...
class TestRpcClient:
    ...
    def test_ping_success(self, rpc_client):
    ...
    def test_submit_command(self, rpc_client):
        """Test command submission."""
        import sys

        job_id = rpc_client.submit_command([sys.executable, "-c", "print('test')"])

        assert isinstance(job_id, str)
        assert len(job_id) > 0

    def test_get_result_running(self, rpc_client):
        """Test getting result of a running job."""
        import sys

        job_id = rpc_client.submit_command([sys.executable, "-c", "import time; time.sleep(1)"])

        # Check immediately - should be running
        result = rpc_client.get_result(job_id)
    ...
    def test_get_result_completed(self, rpc_client):
        """Test getting result of a completed job."""
        import sys

        job_id = rpc_client.submit_command([sys.executable, "-c", "print('hello world')"])
    ...
    def test_wait_for_result_success(self, rpc_client):
        """Test waiting for command completion."""
        import sys

        job_id = rpc_client.submit_command([sys.executable, "-c", "print('completed')"])

        result = rpc_client.wait_for_result(job_id, timeout=5.0)
    ...
    def test_wait_for_result_timeout(self, rpc_client):
        """Test timeout when waiting for command completion."""
        import sys

        job_id = rpc_client.submit_command([sys.executable, "-c", "import time; time.sleep(10)"])

        with pytest.raises(TimeoutError, match="did not complete within"):
            rpc_client.wait_for_result(job_id, timeout=0.5)

    def test_run_command_success(self, rpc_client):
    ...
    def test_is_running(self, rpc_client):
        ...
        job_id = rpc_client.submit_command([sys.executable, "-c", "import time; time.sleep(0.5)"])

        # Job might be running initially
        is_running_initial = rpc_client.is_running(job_id)
        assert isinstance(is_running_initial, bool)

        time.sleep(1.0)
        is_running_after = rpc_client.is_running(job_id)
...
class TestFileOperations:
    ...
    def test_upload_file_success(self, rpc_client, tmp_path):
    ...
    def test_download_file_default_path(self, rpc_client, tmp_path):
    ...
    def test_list_files(self, rpc_client, tmp_path):
    ...
    def test_delete_file(self, rpc_client, tmp_path):
    ...
    def test_delete_all_files(self, rpc_client, tmp_path):
...
class TestErrorHandling:
    ...
    @patch("clabe.rpc._client.RpcClient._call_with_auth")
    def test_server_error_handling(self, mock_call, rpc_client):
    ...
    def test_invalid_job_id(self, rpc_client):
    ...
    def test_file_not_found_download(self, rpc_client):
        """Test downloading non-existent file."""
        with pytest.raises(Exception, match="File not found"):
            rpc_client.download_file("nonexistent_file.txt")

    def test_file_not_found_delete(self, rpc_client):
